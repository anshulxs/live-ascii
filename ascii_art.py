...
class AsciiArt:
    def resize_image(self, image, new_width=70, font_width: float=9.6, font_height: float=16):
    ...
    def pixel_to_ascii(self, image, invert: bool=False):
    ...
    def image_to_ascii(self, image, new_width=70, font_width: float=9.6, font_height: float=16, invert=False):
        ...
        ascii_str = self.pixel_to_ascii(grayscaled_image, invert)
        ascii_image = [ascii_str[i:i+new_width] for i in range(0, len(ascii_str), new_width)]
        return "\n".join(ascii_image)
    
    def image_to_color_ascii(self, image, new_width=70, font_width: int=8, font_height: int=16):
        resized_image = self.resize_image(image, new_width, font_width, font_height)
